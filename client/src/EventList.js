...
class EventList extends Component {
  ...
  async componentDidMount() {
    ...
    const json = await(await fetch(`http://localhost:8080/events`)).json();
    if(json._embedded ? this.setState({events : json._embedded.eventList}) : this.setState({events : []}));
  ...
  render() {
    ...
    const eventListForPdf = events.map(event => {
      return <Text>
        &raquo; {event.name} - {event.type!=null ? event.type.name : event.type} - {event.date} - {event.desc}
      </Text>
    });
